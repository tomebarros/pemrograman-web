...
if ($_POST['pilihan'] == 'pdf') {
  $nama_dokument = 'Laporan Transaksi Nama-';
  ...
  header("Content-Disposition: attachment; filename=Laporan Transaksi Nama-" . $tanggal . ".xls");
  $idpelanggan = $_POST['idpelanggan'];
}
?>
...
Nama Pelanggan : <?php echo namapelanggan($idpelanggan); ?>


<?php 
$cek = $data = mysqli_query($koneksi, "SELECT * FROM transaksi,detailtransaksi WHERE transaksi.idtransaksi = detailtransaksi.idtransaksi AND idpelanggan LIKE '$idpelanggan'");
...
<table border="1" cellspacing="0" width="100%">
  ...
  <tbody>
    ...
    $gt = 0;
    $no = 1;
    $data = mysqli_query($koneksi, "SELECT * FROM transaksi,detailtransaksi WHERE transaksi.idtransaksi = detailtransaksi.idtransaksi AND idpelanggan LIKE '$idpelanggan'");
...
<table width="100%" cellpadding="20" style="text-align: center; font-size:14px; margin-top:50px;">
  <tr>
    <td>
